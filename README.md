...
| `fabric` | the same mod for Fabric 26.2: the Soul Link server and the Fabric client packs. |
...
The Soul Link server and the MCDGG client packs run Fabric on Minecraft 26.2, so the same
mod exists there as `DGGChat-Fabric-<version>.jar`. `dgg-chat-api` and `dgg-chat-core`
are nested unchanged (Loom's `include`); only the platform glue and the mixins differ:

- **Wire-identical payload** on `dggchat:identity`, so a client on either loader
  understands a server on either. Fabric payload types are optional by construction, so
  the NeoForge `optional()` registration has no counterpart.
- **The glyph pipeline is 26.2's.** `GlyphProvider.getGlyph` returns an `UnbakedGlyph`
  whose `bake(Stitcher)` hands a `GlyphBitmap` to the stitcher, and the atlas page arrives
  as a `GpuTexture` rather than being left bound in GL. Frames are written through the
  render device's command encoder, which writes a whole `NativeImage` at an offset, so the
  strip is cut into one image per tile when it decodes.
- **The emote provider is installed in `FontManager.createFontSet`** (the one place a
  font's name and its provider list meet: 26.2's `FontSet` no longer knows its name), and
  the resulting `FontSet` is remembered so the `reload(Set)` hook can recognise it on an
  options change.
- **Chat is rewritten in `ChatComponent.addMessage`**, the private funnel every displayed
  line passes through, because Fabric API has no event that lets a mod replace a signed
  chat line's component. The sender is read from the leading name, which is also what
  worked on NeoForge once No Chat Reports had turned chat into game messages.
- **Rainbow names** hook `Font$PreparedTextBuilder.accept`, the successor to
  `StringRenderOutput`. Animation advances at the head of `GameRenderer.render`.
- **Dropped**: title emotes (`Gui.setTitle` is gone) and the chat depth nudge (26.2's GUI
  is two-dimensional, with no depth to nudge).

Builds against an identity mapping for 26.2 generated by `scripts/setup-262-mappings.sh`
(26.2 ships unobfuscated and publishes no Mojang mappings); see the CI workflows for the
two-step bootstrap. Java 25.
...
| `fabric` | `fabric/` | the mod for Fabric 26.2. Ships as `DGGChat-Fabric-<version>.jar`. |
...
./gradlew :fabric:build           # needs the 26.2 identity mapping, see below
```

Java 21 (NeoForge) and Java 25 (Fabric) are fetched by the toolchain if the machine has
neither. The first `:fabric` build fails at the mappings step but downloads the 26.2 jar;
`bash scripts/setup-262-mappings.sh` then installs the identity mapping and the second
build works. The first `:neoforge` build
...
dependency, and creates a **GitHub Release** with both mod jars attached. The clientdev
pack build downloads `DGGChat-Fabric-*.jar` from the newest Release.
