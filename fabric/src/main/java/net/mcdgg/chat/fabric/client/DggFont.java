...
public final class DggFont {
    ...
    public static final Identifier FONT =
            Identifier.fromNamespaceAndPath(DggChatFabric.MOD_ID, "emotes");
    ...
    /**
     * The {@link net.minecraft.client.gui.font.FontSet} built for {@link #FONT}, remembered
     * because 26.2's FontSet does not know its own name. The mixin on {@code FontSet.reload}
     * fires for every font and needs to tell ours apart; identity is the only handle left.
     */
    private static volatile Object emoteFontSet;

    private DggFont() {}

    /** Called from the FontManager mixin, once per resource reload, with the new FontSet. */
    public static void rememberFontSet(Object fontSet) {
        emoteFontSet = fontSet;
    }

    public static boolean isEmoteFontSet(Object fontSet) {
        return fontSet != null && fontSet == emoteFontSet;
    }
